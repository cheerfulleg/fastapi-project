...
@profile_router.post('/profile', response_model=Profile_Pydantic)
...
@profile_router.get('/profile', response_model=Profile_Pydantic)
async def get_user_profile(user: User_Pydantic = Depends(get_current_user)):
    profile_obj = await Profile.get(user_id=user.id)
    return await Profile_Pydantic.from_tortoise_orm(profile_obj)


@profile_router.put('/profile', response_model=Profile_Pydantic)
