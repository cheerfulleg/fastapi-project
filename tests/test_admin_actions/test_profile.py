...
def test_admin_create_profile_with_invalid_data(client: TestClient, invalid_profile_data: dict,
...
def test_admin_update_profile_with_invalid_data(client: TestClient, get_admin_headers: dict,
                                                invalid_profile_data: dict):
    response = client.put(f'/admin/profile/{PROFILE_ID}', json=invalid_profile_data, headers=get_admin_headers)
    assert response.status_code == 422


def test_admin_update_profile_by_invalid_id(client: TestClient, get_admin_headers: dict, update_profile_data: dict):
    response = client.get(f'/admin/profile/{INVALID_PROFILE_ID}', headers=get_admin_headers, json=update_profile_data)
    assert response.status_code == 404
...
def test_admin_update_profile_by_id_not_admin(client: TestClient, get_default_headers: dict, update_profile_data: dict):
    response = client.put(f'/admin/profile/{PROFILE_ID}', json=update_profile_data, headers=get_default_headers)
    assert response.status_code == 403
