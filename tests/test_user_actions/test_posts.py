...
def test_user_no_profile_create_post(client: TestClient, get_default_headers: dict, create_post_data: dict):
...
def test_profile_create_invalid_post(client: TestClient, invalid_post_data: dict, get_user_with_profile_headers: dict):
...
def test_profile_update_update_post_by_id(client: TestClient, get_user_with_profile_headers: dict, update_post_data: dict):
    response = client.put(
        f"/posts/{POST_ID}",
...
def test_profile_update_update_post_by_invalid_id(client: TestClient, get_user_with_profile_headers: dict, update_post_data: dict):
    response = client.put(
        f"/posts/{INVALID_POST_ID}",
        json=update_post_data,
        headers=get_user_with_profile_headers,
    )
    assert response.status_code == 404
...
def test_profile_update_update_post_by_id_invalid_data(client: TestClient, get_user_with_profile_headers: dict, invalid_post_data: dict):
    response = client.put(
        f"/posts/{INVALID_POST_ID}",
...
def test_profile_delete_post_by_id(
    client: TestClient,
    event_loop: asyncio.AbstractEventLoop,
    get_user_with_profile_headers: dict,
):
